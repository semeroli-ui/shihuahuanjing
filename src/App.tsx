...
export default function App() {
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      // 水墨丹青风格前缀：注入中国传统水墨画视觉指令
      const INK_STYLE_PREFIX = `Traditional Chinese ink wash painting (水墨丹青) style. ` +
        `Monochromatic palette with subtle gradations of black ink on aged rice paper texture. ` +
        `Soft, flowing brushstrokes with wet-on-wet bleeding effects (晕染). ` +
        `Generous use of negative space (留白) to create atmospheric depth and poetic mood. ` +
        `Elegant composition following classical Chinese landscape painting principles: distant mountains in pale wash, middle ground mist, foreground details in concentrated ink. ` +
        `Subtle color accents of vermilion (朱砂), celadon (青绿), or ochre sparingly applied. ` +
        `Aesthetic reminiscent of Song Dynasty masterpiece paintings — serene, contemplative, ethereal. `;

      const styledPrompt = `${INK_STYLE_PREFIX}\n\nScene description: ${visualPrompt.english}`;
      const response = await poeticService.generateImage(styledPrompt) as any;
    ...
    try {
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (errorMsg.includes('429') || errorMsg.includes('频率') || errorMsg.includes('rate')) {
            setVideoStatus('查询频率过高，30秒后自动重试...');
            setTimeout(poll, 30000);
          } else if (errorMsg.includes('525') || errorMsg.includes('SSL')) {
            ...
            // 显示错误但继续重试（网络波动等）
            if (pollCount < 5) {
              setVideoStatus(`视频生成出错: ${errorMsg}，正在重试...`);
              setTimeout(poll, 10000);
            } else {
        ...
        <div className="flex flex-col items-end gap-4">
          ...
          <div className="relative md:hidden">
            <button
              ...
              className="p-3 bg-white/50 backdrop-blur-sm rounded-full border border-zen-ink/10 shadow-sm active:bg-white transition-all"
            >
              <MoreVertical size={18} className="text-zen-ink/60" />
            </button>
              ...
              {showMobileMenu && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute right-0 top-12 w-48 bg-zen-paper paper-texture rounded-2xl shadow-xl border border-zen-ink/10 overflow-hidden z-[56]"
                  >
                    <button
                      onClick={() => { setShowLibrary(true); setShowMobileMenu(false); }}
                      className="w-full flex items-center gap-3 px-5 py-4 hover:bg-zen-vermilion/5 transition-colors border-b border-zen-ink/5"
                    >
                      <Book size={16} className="text-zen-accent" />
                      <span className="text-sm font-serif">藏书阁</span>
                      <span className="ml-auto bg-zen-vermilion/10 text-zen-vermilion text-[10px] px-1.5 rounded-full">{library.length}</span>
                    </button>
                    {isAdmin ? (
                      <button
                        onClick={() => { setShowAdminPanel(true); setShowMobileMenu(false); }}
                        className="w-full flex items-center gap-3 px-5 py-4 hover:bg-zen-ink/5 transition-colors"
                      >
                        <Key size={16} className="text-zen-ink/60" />
                        <span className="text-sm font-serif">管理后台</span>
                      </button>
                    ) : (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-5 py-4 hover:bg-zen-ink/5 transition-colors"
                      >
                        <Key size={16} className="text-zen-ink/40" />
                        <span className="text-sm font-serif">管理员登录</span>
                      </button>
                    )}
                  </motion.div>
                </>
